(() => {
  ...
  function visual(type, label) {
    ...
    const parts = {
      fourway: base + car + '<span class="scene-car car-right"></span><span class="scene-arrow arrow-right">→</span>',
    ...
    return `<figure class="teaching-scene scene-${esc(type)}" role="img" aria-label="${esc(label)}">${parts[type] || base + car}<figcaption>${esc(label)}</figcaption></figure>`;
  }
  ...
  function answer(index) {
    ...
    feedback.innerHTML = `<b>${correct ? 'Correct—keep that rule.' : item.original ? 'Not yet—this rule stays in recovery.' : 'Pause here; this is the rule to keep.'}</b><br>${esc(question.explanation)}${review(question, index, correct)}`;
